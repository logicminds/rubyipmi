...
describe :Sensors do
  ...
  before :each do
    ...
    allow(@sensors).to receive(:`).and_return(sensor_data)
  ...
  describe 'for dell servers' do
    let(:sensor_data) do
      File.open("spec/fixtures/#{@provider}/dell_sensors.txt", 'r') do |file|
        data = file.read
      end
    end
    it "can return a list of sensors" do
    ...
    it "should return a count of sensors" do
      expect(@sensors.count).to eq(45)
    end

    it "should return a list of fan names" do
      expect(@sensors.fanlist.count).to eq(4)
    end

    it 'should return a list of temp names' do
      expect(@sensors.templist.count).to eq(11)
    end

    it 'should return a list of sensor names as an array' do
      expect(@sensors.names).to be_an_instance_of(Array)
      expect(@sensors.names.count).to eq(45)
    end
